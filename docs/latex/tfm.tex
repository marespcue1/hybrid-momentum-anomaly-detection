...
\begin{document}
  ...
  \caption{Rendimientos de las carteras superpuestas J/K de momentum. Fuente: Jegadeesh y Titman (1993).}
  ...
  \caption{Evolución de las caídas catastróficas en estrategias de momentum. Fuente: Daniel y Moskowitz (2016).}
...
El objetivo principal del sistema desarrollado consiste en identificar estructuras de comportamiento dentro de los mercados financieros y detectar observaciones anómalas a partir de información histórica de precios y variables derivadas. Para ello, se propone un enfoque híbrido basado en técnicas de \textit{clustering} y detección de anomalías aplicadas sobre series temporales financieras.

En primer lugar, se presenta el proceso de selección del universo de activos financieros y ETFs utilizados en el estudio, justificando la inclusión de distintos mercados, sectores económicos y factores de riesgo. Posteriormente, se detalla la construcción del conjunto de datos y el proceso de análisis exploratorio de datos (\textit{Exploratory Data Analysis}, EDA), incluyendo las fases de descarga, validación, limpieza y caracterización estadística inicial de las series financieras.
...
\begin{itemize}
\item En primer lugar, se empleará el algoritmo DBSCAN (\textit{Density-Based Spatial Clustering of Applications with Noise}) con el objetivo de detectar y filtrar observaciones anómalas o regiones de baja densidad dentro del espacio de datos.
    
\item Posteriormente, sobre el conjunto de observaciones filtradas, se aplicará el algoritmo K-Means para identificar agrupaciones de activos con comportamientos financieros similares.
\end{itemize}
...
y fondos cotizados (ETFs), con el objetivo de capturar distintos
regímenes de mercado y estructuras sectoriales.

En concreto, se han seleccionado:

\begin{itemize}
\item Acciones del mercado español (IBEX 35), representativas de distintos sectores.
\item Acciones del mercado estadounidense, caracterizadas por alta liquidez y relevancia global.
\item ETFs sectoriales y de mercado, que permiten capturar dinámicas agregadas y factores sistemáticos.
\end{itemize}
...
Por otro lado, el uso de ETFs permite representar de forma eficiente factores de mercado y sectores económicos completos, actuando como aproximaciones agregadas del comportamiento del mercado y reduciendo el impacto del ruido idiosincrático de activos individuales \parencite{madhavan2012exchange}.

Además, la inclusión de activos pertenecientes a distintos sectores económicos se fundamenta en la evidencia de que los retornos presentan estructuras diferenciadas en función del sector, lo que resulta relevante en tareas de segmentación y clustering financiero \parencite{fama1997industry}.
...
En la Tabla~\ref{tab:assets} se presenta el universo de activos financieros utilizado en el desarrollo del sistema propuesto, incluyendo acciones de los mercados español y estadounidense, así como distintos fondos cotizados (ETFs) representativos de sectores, índices y factores de riesgo.
...
\multicolumn{3}{l}{\textbf{Mercado España (IBEX 35)}} \\
...
El uso de retornos, en lugar de precios absolutos, permite comparar activos con escalas de precio completamente diferentes. Por ejemplo, acciones tecnológicas con precios elevados pueden analizarse conjuntamente con ETFs o activos de menor valor nominal sin introducir sesgos derivados de la escala.
...
Los resultados mostraron una fuerte concentración del crecimiento en el sector tecnológico, destacando especialmente NVIDIA, Tesla y el ETF de semiconductores (\texttt{SMH}) como los activos con mayor revalorización acumulada durante el periodo estudiado.
...
\caption{Precios normalizados separando ETFs, activos de españa y activos de EEUU. Fuente: elaboración propia.}
...
\item Los ETFs de renta fija como \texttt{TLT} mostraban baja correlación respecto al resto del mercado.
...
\subsection{Transformación del Espacio de Características: Rolling Z-Score}
...
\subsubsection{Implementación del Rolling Z-Score dinámico}
...
Una vez estandarizadas las variables mediante el estadístico \textit{Rolling Z-Score} (Sección \ref{sec:rolling_zscore}), se tiene una matriz de datos estacionaria donde las distancias Euclídeas entre vectores de activos que son directamente comparables al encontrarse las variables expresadas en una escala homogénea. El siguiente paso en la arquitectura del sistema consiste en la identificación y filtrado del ruido mediante el algoritmo de agrupamiento basado en densidad, DBSCAN.
%\cite{ester1996}.
...
\end{tabular}
\caption{Estadísticas del filtrado de ruido topológico generados por DBSCAN. }
...
K-Means es un algoritmo de clustering no supervisado que particiona un conjunto de observaciones en $K$ grupos minimizando la distancia intra-cluster \parencite{macqueen1967}. Formalmente, el objetivo del algoritmo es minimizar la función:
...
Estas variables no se utilizan en su forma original, sino que han sido previamente transformadas mediante el estadístico Rolling Z-Score (Sección \ref{sec:rolling_zscore}), lo que garantiza su comparabilidad temporal y transversal entre activos.
...
\item \textbf{Indicadores técnicos}: variables derivadas de análisis técnico clásico como RSI y MACD, utilizadas para medir sobrecompra, sobreventa y cambios de tendencia.
...
\subsubsection{Análisis comparativo de estructuras de clustering}
...
\bottomrule
\end{tabular}
\caption{Comparativa de estructuras de clustering para distintos valores de K.}
...
Este cluster se caracteriza por valores negativos en momentum de largo y medio plazo, junto con niveles bajos de indicadores técnicos como RSI y MACD. La volatilidad se sitúa en niveles moderados.
...
Se observa un patrón mixto, con momentum negativo en horizontes largos, pero recuperación en el corto plazo reflejada en MACD y RSI positivos. La volatilidad es elevada.
...
Presenta momentum positivo en todos los horizontes, acompañado de indicadores técnicos favorables y baja volatilidad.
...
Este cluster presenta caídas extremas en momentum de corto plazo, valores muy negativos en MACD y RSI, y niveles elevados de volatilidad y ATR.
...
\begin{itemize}
    \item DBSCAN, encargado de la eliminación de ruido y observaciones anómalas.
